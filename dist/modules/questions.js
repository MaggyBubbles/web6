...
export function distributeQuestions(array) {
    const fiveQuestionLength = 5;
    let fiveQuestions = [];
    ...
    while (fiveQuestions.length < fiveQuestionLength) {
        if (fiveQuestions.length < fiveQuestionLength) {
            fiveQuestions.push(easyQuestions[i]);
        }
        if (fiveQuestions.length < fiveQuestionLength) {
            fiveQuestions.push(mediumQuestions[i]);
        }
        if (fiveQuestions.length < fiveQuestionLength) {
            fiveQuestions.push(hardQuestions[i]);
        }
        i++;
    }
